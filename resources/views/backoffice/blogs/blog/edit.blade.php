                    ...
                    <form action="{{ route('blog.update', $blog->id) }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('cover_img') ? 'has-error' : '' }}">
                            ...
                            <div class="col-md-5">
                                ...
                                @if($blog->cover_img)
                                <br/>
                                <img style="width:200px;" src="{{ asset('uploads/blog_cover_images/'.$blog->cover_img) }}">
                                @endif
                            </div>
                        </div>
                        <div class="form-group {{ $errors->has('body') ? 'has-error' : '' }}">
                            ...
                            <div class="col-md-10">
                                <textarea class="form-control ckeditor" id="body" name="body">{{ htmlspecialchars(old('body', $blog->body)) }}</textarea>
                                @if($errors->has('body'))
                        ...
                        <div class="form-group {{ $errors->has('tag_ids') ? 'has-error' : '' }}">
                            ...
                            <div class="col-md-10">
                                ...
                                <select style="height: 150px;" class="form-control" name="tag_ids[]" multiple>
                                    ...
                                    <option  @foreach($blog->tag as $blog_tag) @if($tag->id == $blog_tag->id) selected="selected" @endif @endforeach value="{{ $tag->id }}">{{ $tag->name }}</option>
                                    @endforeach
                                </select> 
                        ...
                        <div class="form-group {{ $errors->has('active') ? 'has-error' : '' }}">
                            ...
                            <div class="col-md-5">
                                <input data-toggle="toggle" @if($blog->active) checked @endif name="active" type="checkbox"> 
